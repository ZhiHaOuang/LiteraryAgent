...
def build_stage_prompt(
    *,
    ...
    memory_budget = min(10000, context_budget // 3)
    knowledge_budget = min(8000, context_budget // 3)
    prior_budget = max(1000, context_budget - memory_budget - knowledge_budget)
    truncated: list[str] = []
    memory_text = _clip_context(
        memory_context.to_prompt_text(), memory_budget, "memory", truncated
    )
    ...
    prior_text = _clip_context(
        _prior_output_text(prior_outputs), prior_budget, "prior_outputs", truncated
    )
    ...
    text = "\n\n".join(
        [
            ...
            (
                ...
                "Codex is the inference engine, not the product identity. Work only on this stage.\n"
            ...
            (
                ...
                "handoff: structured facts, constraints, open questions, and next-stage inputs.\n"
                "risks: concrete unresolved risks."
            ),
        ]
    )
    return StagePrompt(
